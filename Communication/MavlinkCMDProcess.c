...
static void Cmd22_MAV_CMD_NAV_TAKEOFF( uint8_t port_index, const mavlink_message_t* msg )
{
    ...
    if(false)
    {
       Position_Control_Takeoff_HeightRelative(height);//记得去掉注释
//			Attitude_Control_set_Throttle( height );//记得删除
        if( port->write )
        {
            mavlink_message_t msg_sd;
            mavlink_msg_command_ack_pack_chan(
...
static void Cmd520_MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES( uint8_t port_index, const mavlink_message_t* msg )
{
    const mavlink_command_long_t* msg_rd = (mavlink_command_long_t*)msg->payload64;
    const Port* port = get_Port( port_index );
		uint64_t capability=MAV_PROTOCOL_CAPABILITY_SET_ATTITUDE_TARGET|MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED|MAV_PROTOCOL_CAPABILITY_MAVLINK2;
    mavlink_message_t msg_sd;
//    mavlink_msg_command_ack_pack_chan(
//        1,	//system id
//        MAV_COMP_ID_AUTOPILOT1,	//component id
