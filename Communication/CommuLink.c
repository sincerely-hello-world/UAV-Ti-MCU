...
bool PortRegister( Port port )
{
    ...
    SetMsgRate( p_index, MAVLINK_MSG_ID_ATTITUDE, 50 );
    SetMsgRate( p_index, MAVLINK_MSG_ID_LOCAL_POSITION_NED, 50 );
    SetMsgRate( p_index, MAVLINK_MSG_ID_SCALED_IMU, 25 );
    SetMsgRate( p_index, MAVLINK_MSG_ID_SYS_STATUS, 10 );
    SetMsgRate( p_index, MAVLINK_MSG_ID_EXTENDED_SYS_STATE, 10 );
    SetMsgRate( p_index, MAVLINK_MSG_ID_RC_CHANNELS_SCALED, 5 );
    return true;
}
...
static void CommuLink_SendServer( unsigned int Task_ID )
{
    ...
    if( ++HeartBeat_counter >= 100 )
    {
        ...
        for( uint8_t i = 0 ; i < MAVLINK_COMM_NUM_BUFFERS ; ++i )
        {
            ...
            if(get_current_Mode()>=30)base_mode|=MAV_MODE_FLAG_SAFETY_ARMED;
            custom_mode=get_current_Mode();
            if((get_Attitude_Measurement_System_Status() != Measurement_System_Status_Ready)||(get_Altitude_Measurement_System_Status() != Measurement_System_Status_Ready))
                sys_stutus=MAV_STATE_CALIBRATING;
            else if(get_is_inFlight())
                sys_stutus=MAV_STATE_ACTIVE;
            else
                sys_stutus=MAV_STATE_STANDBY;
