...
/*状态观测器*/
	...
	static inline void update_output_throttle( float throttle , float h )
	{
		...
		{
			onGround_counter = 0;
			if( AccENU.z > 20 && throttle > output_minimum_throttle + 5 )inFlight = true;
//			if (pos_vision.z > Height_t265+2)IsInTakeoff = 1;
//			if( pos_vision.z > Height_t265+10 && throttle > output_minimum_throttle + 5 ){inFlight = true;IsInTakeoff = -1;}
				
		}
		else
		{
			if( hover_throttle < output_minimum_throttle + 2 && \
					in_symmetry_range_float( AccENU.z  , 20 ) && \
					in_symmetry_range_float(  get_VelocityENU().z  , 25 ) \
...
/*模式及期望*/
	...
	bool Attitude_Control_set_Target_Yaw( float Yaw )
	{
		...
		float yaw_err = Yaw - Target_tracker[2].x1;
		while(yaw_err > PI_f)
			yaw_err -= 2*PI_f;
		while(yaw_err < -PI_f)
			yaw_err += 2*PI_f;
...
//		output_throttle = throttle / cos_angle;
	...
	throttle_nonlinear_compensation( rotor_output );
	Quaternion quat = get_Airframe_attitude();
	float pitch = Quaternion_getPitch(quat);
	float roll = Quaternion_getRoll(quat);
	rotor_output[6] = pitch*63.662f + 80;
	rotor_output[7] = roll*63.662f + 60;
	PWM_Out( rotor_output );
}
