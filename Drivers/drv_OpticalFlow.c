    ...
    while( ( UART1->FR & (1<<4) ) == false	)
    {
        ...
        {   //接收包尾
            if( rdata == 0x55 )
            {
                vector3_float vel;
                float laser_height;
                bool laser_height_available = get_Estimated_Sensor_Position_z( &laser_height, default_laser_sensor_index );
                if(  laser_height_available && Flow.valid )
                {
                    float height_deadband = laser_height;
                    if( height_deadband < 5 )
                        height_deadband = 0;
                    ...
                    vel.x = ( (float)Flow.flow_x_integral/integral_time - rotation_compensation_x ) * 1e-4f * ( 1 + height_deadband );
                    vel.y = ( -(float)Flow.flow_y_integral/integral_time - rotation_compensation_y ) * 1e-4f * ( 1 + height_deadband ) ;
                    PositionSensorUpdateVel( default_optical_flow_index, vel, true, -1,0,0);
                }
                else
