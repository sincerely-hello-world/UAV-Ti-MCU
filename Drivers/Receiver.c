...
static RC_Type current_Receiver = RC_Type_PPM;
...
void Receiver_Update( RC_Type _rc, bool connected, float raw_data[16], uint8_t channel_count )
{
    ...
    if( connected )
    {
        ...
        if(Receivers[RC_Type_PPM].available == true)
        {
            current_Receiver = RC_Type_PPM;
        }
        if(Receivers[RC_Type_Sbus].available == true)
        {
            current_Receiver = RC_Type_Sbus;
        }
    }
}
